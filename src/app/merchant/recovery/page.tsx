...
export default function MerchantRecoveryPage() {
  ...
  useEffect(() => {
    const unsub = dbService.subscribeOpportunities((data) => {
      ...
      if (data.length > 0) {
        // Keep currently selected or set first
        setSelectedOpp((prev) => {
  ...
  const handleExecuteRecovery = async () => {
    ...
    try {
      ...
      if (data.success) {
        // Updated state will sync via subscription
      }
    } catch (e) {
  ...
  return (
    <div className="bg-[#E5E5E5] text-[#191c1e] font-sans min-h-screen py-8 px-4 flex justify-center selection:bg-[#D4FF00] selection:text-black">
      <div className="bg-[#2a2a2a] w-full max-w-[1440px] min-h-[920px] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative text-white border border-white/10">
        <MerchantNav />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 pt-2.5 border-t border-white/10 flex items-center justify-between text-xs">
                        <span className="text-white/60 truncate max-w-[220px]">{opp.failureType}</span>
